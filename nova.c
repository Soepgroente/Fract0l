...
/*   Updated: 2023/06/11 15:56:50 by vvan-der         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

bool	black_hole(t_complex z, t_complex z_prev)
{
	t_complex	a;
	t_complex	b;
	t_complex	c;
	...
	if (fabsl(z.x - a.x) < tol && fabsl(z.i - a.i) < tol)
		return (true);
	if (fabsl(z.x - b.x) < tol && fabsl(z.i - b.i) < tol)
		return (true);
	if (fabsl(z.x - c.x) < tol && fabsl(z.i - c.i) < tol)
		return (true);
	if (fabsl(z.x - z_prev.x) < tol && fabsl(z.i - z_prev.i) < tol)
		return (true);
	return (false);
}
